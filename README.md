...
Косвенные метрики забрали вес у более значимых: `correctness` и `format`. Например, в течение работы, улучшалось значение метрики `grpo_reasoning_steps_reward`. То есть по косвенным метрикам, модель училась "лучше рассуждать", но это было во вред правильности ответов.
<p align="center">
...
2. Поощрение формата:
    ...
    a. `grpo_formatting_reward`. Полное соблюдение формата. Вес 0.5
    
    b. `grpo_tag_count_reward`. Промежуточная функция для поощрения соблюдения формата: за каждый использованный тег даём по `0.5/4 = 0.125`
3. `grpo_length_efficiency_reward`. Вышеописанная функция. Поощряем короткие, но правильные решения
...
## Attempt #3. Результаты
К сожалению, на полное обучение времени не хватило, для последующих результатов взята версия, после `1250` шагов из `7473`.

В результате: `correctness: 983/1319`, `format: 1305/1319`.

<p align="center">
  <img src="./media/grpo_correctness_reward.png" alt="Correctness" width="45%"/>
  <img src="./media/grpo_formatting_reward.png" alt="Format" width="45%"/>
</p>
...
Ура победа, товарищи! Конечно, в сравнении с baseline того же размера, выглядит уже не так привлекательно, но рост всё равно есть. Также, есть уверенность, что при полном обучении, как минимум `format reward` будет доведен до максимума.

| | Before (3b)  | After (Tuned) | % |
| ------------- | ------------- | ------------- | ------------- |
| Correct Format | 1077/1319  | 1305/1319  | +21% |
| Correct Answer | 823/1319  | 983/1319  | +19% |

## Другие идеи
После первой попытки, когда стало понятно, что нужна `reward`-функция, учитывающая длину ответа, появилась идея дообучить `BERT` для того, чтобы он выдавал количество токенов, требуемое для ответа, и далее, можно было бы ставить это число, как ограничение (через `reward` механизм). 

Я обучал модель `roberta-large`, на датасете [EleutherAI/hendrycks_math/prealgebra](https://huggingface.co/datasets/EleutherAI/hendrycks_math/blob/main/README.md). Однако, эта идея не увенчалась успехом: погрешность в результатах модели была слишком большая (`25%`).

## Итоговые итоги
При помощи GRPO получилось улучшить результаты большей модели в решении математических задач. На маленьком датасете получить одновременно и соблюдение формата, и улучшение точности ответов не удалось. Думаю, так получается из-за структуры модели: меньшая была получена дистиляцией из большей Qwen. Ну и при этом как будто модель становится не очень гибкой к обучению: чем меньше весов, тем легче нарушить равновесие. 

Видимо, GRPO не имеет смысла для маленьких моделей - не хватает емкости, чтобы тянуть сложные reward-функции. В таких случаях лучше делать SFT на траекториях большой reasoning-модели или получать меньшую модель через дистилляцию. Это проще, стабильнее и даёт лучшее качество без лишних мучений.